...
def get_1D_filter(mu, poly, order=0):
    ...
    # Populate second half based on symmetry (centred filters, leave central col out)
    if symmetry == 'S':
...
def get_3D_filter(mu, nu, rho, polyx, polyy, polyz, order):
    ...
    '''
    folder = pkg_resources.resource_filename('savitzkygolay', 'filters/')
    fn = "{}3DSavitzkyGolayFilters/CC_{:03d}x{:03d}x{:03d}_{:03d}x{:03d}x{:03d}.dat".format(folder, mu, nu, rho,
                                                                                            polyx, polyy, polyz)
    f = open(fn, 'r')
